...
# to assign the next lane for scheduling
ambTrack = 0
maxamb = -1
...
# array to store green times of other lanes
greentimeArray=[]

# array to keep track of one cycle
...
vehicles = {'right': {0:[], 1:[], 2:[], 'crossed':0}, 'down': {0:[], 1:[], 2:[], 'crossed':0}, 'left': {0:[], 1:[], 2:[], 'crossed':0}, 'up': {0:[], 1:[], 2:[], 'crossed':0}}   # {0:[], 1:[], 2:[]} means the lane for bike , lane1 , lane2 on a single side
...
vehicleCountTexts = ["0", "0", "0", "0"]   # displays the vehicles which have crossed the intersection 
...
def repeat():
    ...
    while(signals[currentGreen].green>0):   # while the timer of current green signal is not zero
        ...
        if(signals[currentGreen].green==detectionTime):    # set time of next green signal 
...
def generateVehicles():
    global countambulance
    while(True):
        vehicle_type = random.randint(0,5)
        
        if(vehicle_type==5):
            countambulance = countambulance +1
            if(timeElapsed-ambulancecount[len(ambulancecount)-1]<30 and countambulance!=1):
                vehicle_type = random.randint(0,4)
            else:
                ambulancecount.append(timeElapsed)   
        if(vehicle_type==4):
            lane_number = 0 # for bikes
        else:
            lane_number = random.randint(0,1) + 1  # could be 1 / 2
        will_turn = 0
        if(lane_number==2):   # vehicles in the lane near the divider and vehicles in lane 0,1 will always go straight i.e bikes + lane1
            temp = random.randint(0,4) # randomness
            if(temp<=2):
                will_turn = 1
            elif(temp>2):
                will_turn = 0
        temp = random.randint(0,999)  #randomness 
        direction_number = 0
        a = [400,800,900,1000]
        if(temp<a[0]):
